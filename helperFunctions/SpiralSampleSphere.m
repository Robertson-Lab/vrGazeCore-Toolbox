...
if nargin<2 || isempty(vis), vis=true; end
...
%tlb2018 - convert cartesian coordinates to spherical (return only azimuth &
%elevation)
sph = [];

for i = 1:length(V)
    [sph(i,1), sph(i,2), ~] = cart2sph(V(i,1),V(i,2),V(i,3));
end

return;

filename = "sphereCoords.txt";
fid = fopen(filename,'w');

for i = 1:length(V)
    [sph(1), sph(2),~] = cart2sph(V(i,1),V(i,2),V(i,3));
    if i == length(V)
        fprintf(fid,"%f,%f",[rad2deg(sph(1)),rad2deg(sph(2))]);
        fclose(fid);
    else
        fprintf(fid, "%f,%f\r\n",[rad2deg(sph(1)),rad2deg(sph(2))]);
    end   
end
