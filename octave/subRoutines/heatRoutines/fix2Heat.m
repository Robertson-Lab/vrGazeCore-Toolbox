...
% findFixations needs to be run before this step.
    ...
    fprintf(fileID, '\nStart Time: %s\nParams:\n', datestr(clock())); %print start datetime to log
    ...
    for iParam = 1:length(paramsList) % log all of the params
        ...
        fprintf(fileID, '%s: %s\n', cellstr(paramsList{iParam}), cellstr( params.(char(paramsList{iParam})) ) ); %print script version to log
    end
    ...
    fprintf(fileID, '\ngroupHeat = %s', cellstr(groupHeat)); %print script version to log
    %% Process data
