%% DP's Notes:
%  PURPOSE: pull out & name useful variables from filtered raw data
%  INPUTS: ds_filtered (filtered raw data)
%  OUTPUTS: gaze_ecc
...
% eye+head)
    ...
    if params.unityProjectVersion == 1 % Newer projects: SNAPI, EMAX, etc
...
%         % tlb - new method?
    ...
    else % Active-Passive and older projects require a different expansion
        
        gazeX = gazeX-0.5;
        gazeX_deg( gazeX>=0 ,: ) = rad2deg ( 1.9 * atan( 1* gazeX(gazeX>=0)));
        gazeX_deg( gazeX< 0 ,:) = - rad2deg (  1.9 * atan( -1* gazeX(gazeX<0)));

        gazeY = gazeY-0.505;
        gazeY_deg( gazeY>=0 ,: ) = rad2deg ( 1.9 * atan( 1 * gazeY(gazeY>=0)));
        gazeY_deg( gazeY< 0 ,:) = - rad2deg ( 1.9 *  atan( -1* gazeY(gazeY<0)));
